...
bool BVH::getIntersection( Ray& ray, IntersectionInfo* intersection, bool occlusion, ShadeRec* sr) const {
  ...
  while(stackptr>=0) {
    ...
    if( node.rightOffset == 0 ) {
      for(uint32_t o=0;o<node.nPrims;++o) {
        ...
        const GeometricObject* obj = (*build_prims)[node.start+o];
...
//Only modification here
        ...
        if (!occlusion)
        {
          hit =  obj->hit(ray,current.t, *sr,current);
          //this shit will be replaced! don't set sr in here!
          //sr->hit_an_object = hit;                 
        }else{
          hit =  obj->shadow_hit(ray,current.t,current);
        }
...
BVH::BVH(std::vector<GeometricObject*>* objects, uint32_t leafSize)
