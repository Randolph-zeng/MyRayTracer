...
RGBAPixel
GlossySpecular::f(const ShadeRec& sr, const Vector3D& wo, const Vector3D& wi) {
	RGBAPixel 	L(0.0,0.0,0.0);  				
...
RGBAPixel
GlossySpecular::sample_f(const ShadeRec& sr, const Vector3D& wo, Vector3D& wi, float& pdf) {
...
RGBAPixel
GlossySpecular::rho(const ShadeRec& sr, const Vector3D& wo) {
