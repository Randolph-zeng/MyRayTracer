...
RGBAPixel
Lambertian::f(const ShadeRec& sr, const Vector3D& wo, const Vector3D& wi)  {
	return (cd * kd * invPI);
}
...
RGBAPixel
Lambertian::sample_f(const ShadeRec& sr, const Vector3D& wo, Vector3D& wi, float& pdf)  {
...
RGBAPixel
Lambertian::rho(const ShadeRec& sr, const Vector3D& wo)  {
	return (cd * kd);
}
